...
**Gate the LOOP on budget, not just the engine — see the `credit-pacing` skill.** `pick-review-engine` only chooses *which engine runs one call*; it does NOT decide whether to keep looping, and that's how a run chomps into overage. Before each new review round, re-check budget (per `credit-pacing`): if the Codex pool is tapped, stop the loop and either switch to AGY or schedule the remaining rounds for the window reset — don't grind into overage. In an unattended/graveyard run, export `BUDGET_FAIL_CLOSED=1` so a missing or stale CodexBar reading falls back to AGY instead of spending Codex blind.
...
**Cross-family synthesis panel (`REVIEW_SYNTHESIS=1`).** For a reconciled second opinion instead of a single engine's vote, set `REVIEW_SYNTHESIS=1` and the `review` script runs **both** Codex (GPT) and AGY (Gemini) on the diff, then synthesizes their findings into three parts — **AGREE** (both raised → highest confidence, fix first), **DIVERGE** (only one raised → judgment needed), **TRANSCENDENT READ** (the deeper theme neither named outright). This is the multi-advisor-synthesis discipline (#150) applied to PR review: a multi-advisor pass reports one reconciled view, not a vote. It's **opt-in and additive** — unset, the script behaves exactly as before — and it **spends both pools per round**, so mind credit-pacing (it falls back to single-engine if both engines aren't installed). The `[P1]/[P2]/[P3]` tags survive into the synthesis, so the judgment gate still parses.
